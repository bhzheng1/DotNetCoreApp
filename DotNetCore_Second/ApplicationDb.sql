...
IF @@ERROR = 3702
	RAISERROR('[ApplicationDb] database cannot be dropped because there are still other open connections', 127, 127) WITH NOWAIT, LOG;
GO
...
CREATE DATABASE ApplicationDb
	ON primary (NAME = 'ApplicationDb_Data', FILENAME = N'/var/opt/mssql/data/ApplicationDb_Data.mdf', SIZE = 10, FILEGROWTH = 8)
	LOG ON (NAME = 'ApplicationDb_Log', FILENAME= N'/var/opt/mssql/data/ApplicationDb_Log.ldf', SIZE = 2, FILEGROWTH = 96);
GO

PRINT '';
PRINT '*** Checking for ApplicationDb Database';
...
BEGIN
        PRINT '*******************************************************************************************************************************************************************'
+char(10)+'********ApplicationDb Database does not exist.  I cannot go ahead.*********'
+char(10)+'*******************************************************************************************************************************************************************';
        SET NOEXEC ON;
END
GO
...
BEGIN
        INSERT [dbo].[Colors]
                ([Id], [Name], [Description])
        VALUES
                (1, N'Gray', N'Characterized by progressive silvering of the colored hairs of the coat. Most gray horses have black skin and dark eyes; unlike many depigmentation genes, gray does not affect skin or eye color')
			,
...
BEGIN
        INSERT [dbo].[Horses]
                ([Id], [Name], [ColorId], [SireId], [DamId], [RaceStarts], [RaceWins], [RacePlace], [RaceShow], [Earnings])
        VALUES
                (1, N'Camarero', 2, NULL, NULL, 76, 73, 2, 0, 43553)
			,
